...
{	
	...
	// '0' is used to convert into achii numbers.
	inputs[measurements_count] = (!INPUT_PIN) + '0';

	measurements_count++;
	
	if(measurements_count > NUMBER_OF_MEASURINGS - 1)
	{
		measurements_count = 0;
		ENABLE_INPUT_INTERRUPT;
...
//used to reset if start bits are never recived.
ISR (TIMER0_OVF_vect)
{
	first_start_bit = true;
	DISABL_TIMER0_OVERFLOW_INT;	
